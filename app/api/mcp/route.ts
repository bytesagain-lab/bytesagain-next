  ...
  if (method === 'tools/list') {
    return NextResponse.json({
      ...
      result: { tools: [
        ...
        { name: 'get_skill',
          description: 'Fetch complete details for a single skill by its exact slug. Use ONLY when the user selects a specific skill from search_skills or popular_skills results and wants full details. Do NOT call this for every item in a list — only on explicit user selection. Returns: name, description, category, tags, version, author, downloads, stars, install command, homepage URL, source repo URL. If the slug does not exist, returns error {code: 404, message: "Skill not found"} — in that case, call search_skills with a related keyword. Never guess or construct slugs; always use slugs from prior tool results. One call per user-selected skill.',
          inputSchema: { type: 'object', properties: {
        ...
        { name: 'popular_skills',
          description: 'Return the top N most-downloaded AI agent skills across all categories, ranked by download count descending. Each result includes: slug, name, one-line description, category, downloads, stars. Use when: user asks for trending/popular/best skills with no specific topic, or during onboarding. Do NOT use when user has a specific task — use search_skills instead. After showing results, call get_skill only if user selects one for deeper details. Returns empty list if database is unavailable (do not retry more than once). Default limit 20, max 50.',
          inputSchema: { type: 'object', properties: {
