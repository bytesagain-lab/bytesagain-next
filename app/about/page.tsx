...
export default function AboutPage() {
  return (
    <div style={{ maxWidth: 750, margin: '60px auto', padding: '0 20px' }}>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: '1.1em', lineHeight: 1.8, marginBottom: 24 }}>
        BytesAgain is a curated directory of AI agent skills — the best tools for Claude, ChatGPT, Cursor, and every modern AI agent.
      </p>

      <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-primary)', borderRadius: 16, padding: 32, marginBottom: 24 }}>
      ...
      <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-primary)', borderRadius: 16, padding: 32, marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 16, marginTop: 16 }}>
          {[
            ...
            <div key={label} style={{ textAlign: 'center', padding: 16, background: 'var(--bg-primary)', borderRadius: 10 }}>
              ...
              <div style={{ color: 'var(--text-muted2)', fontSize: '.85em', marginTop: 4 }}>{label}</div>
            </div>
          ))}
        </div>
      </div>

      <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-primary)', borderRadius: 16, padding: 32 }}>
        <h2 style={{ margin: '0 0 12px', fontSize: '1.2em' }}>Contact</h2>
        <p style={{ color: 'var(--text-muted)', lineHeight: 1.8, margin: '0 0 16px' }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          ...
          <a href="/feedback?type=partner" style={{
            padding: '9px 20px', borderRadius: 8, fontSize: '.88em', fontWeight: 600,
            background: 'transparent', border: '1px solid #333', color: 'var(--text-secondary)', textDecoration: 'none',
          }}>Partner with Us</a>
        </div>
      </div>
    </div>
