  ...
  return (
    <div ref={ref} style={{ position: 'relative', maxWidth: 520, margin: '0 auto 32px' }}>
      <div style={{ position: 'relative' }}>
        ...
        />
        {loading && (
          <span style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted3)', fontSize: '.8em' }}>…</span>
        )}
      </div>
        ...
        <div style={{
          ...
          {results.map(skill => (
            <a key={skill.slug}
              ...
              style={{ display: 'block', padding: '12px 16px', borderBottom: '1px solid var(--border-secondary)', textDecoration: 'none' }}
              onMouseDown={e => e.preventDefault()}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: '.95em' }}>{skill.name || skill.slug}</span>
                <span style={{ color: 'var(--text-muted3)', fontSize: '.72em' }}>
              ...
              <div style={{ color: 'var(--text-muted2)', fontSize: '.8em', marginTop: 4 }}>{skill.description?.slice(0, 70)}…</div>
            </a>
          ))}
        </div>
      )}
      ...
      {open && results.length === 0 && !loading && query.trim() && (
        <div style={{ position: 'absolute', top: '110%', left: 0, right: 0, background: 'var(--bg-secondary)', border: '1px solid var(--border-primary)', borderRadius: 10, padding: 16, textAlign: 'center' }}>
          <div style={{ color: 'var(--text-muted3)', fontSize: '.9em', marginBottom: 10 }}>No skills found for &quot;{query}&quot;</div>
